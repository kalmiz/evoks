...
exports.all = function all(fn) {
  ...
  Law
  .find({ deletedAt: null })
  .select('id lawId friendlyURL mediaTitle tag participants votes createdAt updatedAt closingAt publishedAt deletedAt status open closed links author authorUrl')
...
/**
 * Get Law form `friendlyURL` string
 *
 * @param {String} friendlyURL Law's `friendlyURL`
 * @param {Function} fn callback function
 *   - 'err' error found while process or `null`
 *   - 'law' found item or `undefined`
 * @api public
...
exports.getByFriendlyURL = function get(friendlyURL, fn) {
  var query = { friendlyURL: friendlyURL, deletedAt: null };

  log('Looking for law %s', friendlyURL);
  Law
  .findOne(query)
  .populate('tag')
  .exec(function (err, law) {
    ...
    if (!law) {
      log('Law %s not found', friendlyURL);
      return fn(null);
    }
    log('Delivering law %s', law.id);
    fn(null, law);
