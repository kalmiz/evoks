...
page('/topic/:friendlyURL', citizen.optional, loadByFriendlyURL, function(ctx, next) {
  function validUrl() {
    var pathname = window.location.pathname;
    return pathname == '/' ||  /^\/(law|proposal|topic)/.test(pathname);
  }
  ...
  if (!ctx.law) {
    log('Law %s not found', ctx.params.friendlyURL);
    return next();
  }
...
/*
 * Load homepage data by friendlyURL
 *
 * @param {Object} ctx page's context
 * @param {Function} next callback after load
 * @api private
...
function loadByFriendlyURL (ctx, next) {
  log('fetch for %s', ctx.params.friendlyURL);

  request
  .get('/api/topic/' + ctx.params.friendlyURL)
  .end(function(err, res) {
    if (!err && res.status == 404) return ctx.law = null, next();
    if (err || !res.ok) return log('Found error: %s', err || res.error);
    ctx.law = res.body;
