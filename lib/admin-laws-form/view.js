...
function LawForm(law) {
  ...
  richtext(summary);
  var shortsummary = this.find('textarea[name=shortsummary]');
  richtext(shortsummary);

  var pros = this.find('textarea[name=pros]');
  richtext(pros, {toolbar:{video: false, image: false}});
  var cons = this.find('textarea[name=cons]');
  richtext(cons, {toolbar:{video: false, image: false}});
