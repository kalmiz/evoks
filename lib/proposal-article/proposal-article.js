...
function ProposalArticle (proposal, reference) {
  ...
  this.insertMeta({
    name: 'og:description',
    content: proposal.shortsummary,
    id: 'ogDescription'});
  this.insertMeta({
    name: 'og:title',
    content: proposal.mediaTitle,
    id: 'ogTitle'
  });
}
 ...
 * @param {String} content - meta tag content property
 * @param {String} id - id of the tag
 * @return {Object} the inserted node
 * @api private
 */

ProposalArticle.prototype.insertMeta = function(param) {
  var el = document.getElementById(param.id);

  if(el){
    el.content = param.content;
  } else {
    delete el;
